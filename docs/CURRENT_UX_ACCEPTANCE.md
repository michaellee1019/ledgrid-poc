...
These criteria apply to the current product at `/`, `/control`, and `/painter`,
including their guarded Composer handoffs.
...
- **Live**: content actively sent to the wall. Scene/animation takeover always
  requires **Check & activate in Composer**; queued work is Pending, not Live.
  Painter's distinct direct action remains labeled **Mirror to wall**.
...
|---|---|---|---|
| LIVE-01 | P0 | Opening Studio, Advanced, or Painter never takes over the wall. | Begin with a known live animation. Load each page separately. The live animation and its parameters remain unchanged; the page sends no start, stop, parameter, painter-frame, or scene-mutation request as a side effect of loading. Status reads and isolated preview requests are allowed. |
| LIVE-02 | P0 | Browsing and editing are private by default. | On every workspace, selecting an animation or preset, opening Adjust, loading a scene, or editing a painter cell leaves the known live animation unchanged until an explicit live action is invoked. The UI identifies the work as Draft or Preview. |
| LIVE-03 | P0 | Going live is explicit and accurately reflected. | Scene or animation takeover is available only through **Check & activate in Composer**. Queue acceptance is Pending, never Active. Active requires a correlated exact-identity observation; cancellation and rollback require their own correlated terminal result. Painter's distinct direct action remains labeled **Mirror to wall**. |
| LIVE-04 | P0 | Live output is reversible without losing work. | While Painter is live, **Stop / Return to draft** is visible without scrolling. The global bar always exposes **Stop live output**. Activating it ends live output, returns the editor UI to Draft, and retains the current canvas. |
...
| IA-01 | P1 | The product is organized around present-day tasks rather than accumulated controls. | Studio exposes Library, Now Playing / Adjust, Compose, and System as named task areas. Painter is a named creation workspace. Advanced is clearly labeled for diagnostics and direct system control. Each area is reachable within one navigation interaction. |
...
| DISC-05 | P1 | Recent work is easy to resume. | A deliberate item interaction (open Adjust, Preview, or Composer activation) adds the item to Recent; page load alone does not. Recent is ordered newest first, de-duplicated, capped at 12 items, survives reload in the same browser, and can be filtered without changing live output. |
...
| PARAM-03 | P1 | Live tuning cannot silently target the wrong animation. | Adjust initially changes only draft values. Live parameter updates are enabled only when the adjusted animation is the observed live animation and the UI shows that relationship; otherwise the person must use guarded Composer activation first. |
...
| CAP-01 | P0 | Existing functionality remains reachable after the reorganization. | The running UX exposes guarded animation/scene activation in Composer, contextual adjustment, animation speed/wall mood/plant behavior, scene draft create/update/delete, painter drawing/masks/presets/mirror, ordinary emoji catalog entries in Composer, component catalog/tests/calibration, performance and receiver diagnostics, raw status, and Tetris controls. No takeover action silently calls a rejected legacy route. |
...
| RESP-01 | P1 | Core workflows remain usable on a phone-sized viewport. | At 390×844 there is no horizontal document overflow on all three pages. Live status, the live/stop action, task navigation, and the active workspace remain reachable and controls do not overlap. |
| CONS-01 | P1 | Shared concepts use consistent language and hierarchy. | The same visible terms—Draft, Preview, Check & activate in Composer, Pending, Active, Stop / Return to draft, Now Playing, and Advanced—mean the same thing across workspaces. Primary actions are visually distinct from secondary selection/editing actions. |
...
| CONS-03 | P0 | Every mutation has one clear owner and no competing duplicate action. | Studio Now Playing is the sole owner of animation preset load/save/delete. Advanced links there instead of repeating those actions. The persistent live bar is the sole general Stop control. Loading an animation preset, mask preset, or scene preset changes only a draft; it never invokes a live/apply endpoint. Each slider has a visible label and visible current value. |
...
6. Draw in Painter while the prior animation remains live; then mirror to the
   wall, stop/return to draft, and prove the draft remains.
...
| Painter preview represents the physical installation | PHYS-02 |
...
| Preserve scenes, tests, calibration, diagnostics, raw status, Tetris, Painter masks/presets, and the ordinary emoji catalog | CAP-01 |
