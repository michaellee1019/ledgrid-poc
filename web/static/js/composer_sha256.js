/* Portable SHA-256 for worker origins where Web Crypto is unavailable. */
...
function bytesOf(value) {
    ...
    throw new TypeError('SHA-256 input must be an ArrayBuffer or typed byte view.');
}
...
export async function sha256Bytes(value, cryptoProvider = (
    typeof self !== 'undefined' ? self.crypto : undefined
)) {
    const input = bytesOf(value);
    if (cryptoProvider?.subtle) {
        return new Uint8Array(await cryptoProvider.subtle.digest('SHA-256', input));
    }
    return sha256Portable(input);
}
