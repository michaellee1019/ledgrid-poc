(() => {
  ...
  // Fragment navigation is deliberately presentation-only: it never touches draft, preview, or live state.
  function focusComposerSection() {
    const target = location.hash ? document.querySelector(location.hash) : null;
    if (target && target.matches('.composer-card')) target.focus({preventScroll:true});
  }
  ...
  $('#goLive').addEventListener('click', goLive); $('#stopScene').addEventListener('click', stopScene); render(); queuePreview(); fetch(`${api}/status`).then((response)=>response.json()).then(status); fetch(`${api}/draft`).then(async (response)=>{ const body=await response.json(); if(body.draft){state.recovery=body.draft; $('#restoreDraft').disabled=false; $('#recoveryCard').hidden=false; publishComposerExplanation();} else if(!response.ok){$('#restoreDraft').disabled=true;$('#recoveryCard').hidden=false;$('#recoveryMessage').textContent=body.error || 'Working draft can only be discarded.'; publishComposerExplanation();} }); looks().then((result) => { state.looks=result.looks; renderLooks(); }).catch((error) => { $('#lookMessage').textContent=error.message; }); refreshLibrary().catch((error)=>{ $('#starterMessage').textContent=error.message; }); fetch(`${api}/components`).then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.error || 'Overlay choices are unavailable.'); return body; }).then((body) => { state.components = body.choices; body.choices.forEach((choice) => { defaults[choice.slot_id] = { slot_id: choice.slot_id, component_id: choice.component_id, enabled: true, opacity: choice.component_id === 'conway_life' ? 190 : 208, strip: 0, led: choice.component_id === 'clock_overlay' ? -8 : 0, stale: 'hold', parameters: structuredClone(choice.parameters) }; }); render(); }).catch((error) => { $('#previewStatus').textContent = error.message; publishComposerExplanation(); });
})();
