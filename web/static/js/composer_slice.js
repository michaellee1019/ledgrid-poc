(() => {
  ...
  function renderStatus(payload) {
    ...
    $('#operationMessage').textContent = status.last_error || (status.armed ? 'Changes publish immediately.' : 'Changes are held until Go Live.');
  }
  ...
  async function edit(event) { state.lastControl = event?.target?.id || null; const next = sceneFromControls(); state.dirty = true; try { await submit(next, {rememberEdit: true}); } catch (error) { $('#operationMessage').textContent = error.message; } }
  ...
  async function openItem(item) {
    try {
      ...
      else { const starterResponse = await fetch(`${api}/starters/${item.id}`); const starter = (await starterResponse.json()).starter; await submit(starter.scene, {builtin: true}); $('#sceneName').value = starter.name; }
  ...
  async function check() { try { const response = await fetch(`${api}/check`, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({origin: 'composer', scene: sceneFromControls()})}); const result = await response.json(); $('#checkMessage').textContent = response.ok ? 'This is advisory; it does not change output.' : (result.error || 'Check could not complete.'); const details = $('#checkDetails'); details.replaceChildren(); [['Scene identity', identity(result.basis)], ['Connection', result.status?.connected ? 'Connected' : 'Disconnected'], ['Publication', result.status?.armed ? 'Immediate when edited' : 'Held until Go Live']].forEach(([term, description]) => { const entry = document.createElement('div'); entry.innerHTML = `<dt>${term}</dt><dd>${description}</dd>`; details.append(entry); }); if (result.status) renderStatus(result); openDialog($('#checkDialog')); } catch (error) { $('#operationMessage').textContent = error.message; } }
  ...
  function wire() {
    ['#backgroundGain','#curtainDensity','#foldDepth','#glowIntensity','#animationChoice','#lifeSeed','#lifeRate','#clockEnabled','#clockOffset','#previewPalette','#wallPace','#sceneLuminance'].forEach((selector) => $(selector).addEventListener('change', edit));
