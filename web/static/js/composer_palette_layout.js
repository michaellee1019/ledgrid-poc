/* Desktop-only palette arrangement. This never reads or writes Composer scene state. */
(() => {
  'use strict';
  ...
  const paletteDefinitions = [
    ...
    ...[...controlsWorkspace.querySelectorAll('.inspector')].map((node) => {
      ...
      const label = node.querySelector('.eyebrow')?.textContent?.trim() || document.getElementById(headingId)?.textContent?.trim() || headingId;
  ...
  function ensurePaletteFrame(id) {
    ...
    node.querySelector('[data-palette-action="drag"]').draggable = true;
  }
  function action(id, actionName) {
    const current = groupFor(id);
    if (actionName === 'hide') { remove(id); layout.hidden.push(id); save(); render(); document.querySelector('#paletteChooser summary')?.focus(); say(`${title(id)} hidden. Use Palettes to restore it.`); return; }
  ...
  function setVisibility(id, visible) {
    if (visible) { layout.hidden = layout.hidden.filter((value) => value !== id); layout.groups.push([id]); layout.widths.push(1); }
    else { remove(id); layout.hidden.push(id); }
    save(); render(); focusAction(id, visible ? 'collapse' : 'hide'); say(`${title(id)} ${visible ? 'restored' : 'hidden'}.`);
  }
  function reset() { layout = defaultLayout(); try { localStorage.removeItem(storageKey); } catch (_) {} render(); focusAction('background', 'collapse'); say('Palette layout reset to the documented default.'); }
  function moveDrop(id, targetGroup, targetIndex) {
    ...
    save(); render(); focusAction(id, 'drag'); say(`${title(id)} moved into a palette stack.`);
  }
  ...
  function updateColumnWidth(index, delta) {
    ...
    layout.widths[index] = clamp((layout.widths[index] || 1) + shift, .65, 2.2); layout.widths[index + 1] = clamp((layout.widths[index + 1] || 1) - shift, .65, 2.2); board.style.gridTemplateColumns = gridColumns();
  }
  function updateHeight(id, delta) { layout.heights[id] = clamp((layout.heights[id] || 300) + delta, 180, 900); nodes.get(id).style.setProperty('--palette-height', `${layout.heights[id]}px`); }
  function makeControls() {
    boardControls = document.createElement('div'); boardControls.className = 'palette-arrangebar';
    const copy = document.createElement('p'); copy.textContent = 'Arrange palettes — drag to stack or use each title bar.';
  ...
  function renderPaletteChooser() {
    ...
    paletteDefinitions.forEach(([id]) => {
      const visible = !layout.hidden.includes(id); const button = document.createElement('button'); button.type = 'button'; button.className = 'button secondary'; button.textContent = `${visible ? 'Hide' : 'Show'} ${title(id)}`; button.setAttribute('aria-pressed', String(visible)); button.addEventListener('click', () => setVisibility(id, !visible)); list.append(button);
    });
  }
  function render() {
    if (!desktop.matches) { teardownDesktop(); return; }
    board.classList.add('palette-board'); if (!boardControls) makeControls(); board.replaceChildren(boardControls, controlsWorkspace); board.style.setProperty('--palette-columns', layout.groups.length); board.style.gridTemplateColumns = gridColumns();
  ...
  function teardownDesktop() {
    ...
    original.slice().reverse().forEach(({node, parent, next}) => {
      node.classList.remove('palette-shell', 'is-collapsed', 'is-dragging'); node.style.removeProperty('--palette-height'); node.querySelector('.palette-header-controls')?.remove(); node.querySelector('.palette-resize-handle')?.remove();
      parent.insertBefore(node, next?.parentNode === parent ? next : null);
    });
    board.classList.remove('palette-board'); board.style.removeProperty('--palette-columns'); board.style.removeProperty('grid-template-columns'); boardControls = undefined; announce = undefined; board.replaceChildren(...originalBoardChildren);
  }
  ...
  board.addEventListener('dragstart', (event) => { const grab = event.target.closest('.palette-grab'); if (!grab) return; draggedId = grab.dataset.paletteId; nodes.get(draggedId).classList.add('is-dragging'); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', draggedId); });
  ...
  board.addEventListener('dragover', (event) => { const stack = event.target.closest('.palette-stack'); if (!stack || !draggedId) return; const group = layout.groups[Number(stack.dataset.groupIndex)]; if (group.includes(draggedId) && group.length === 1) return; event.preventDefault(); event.dataTransfer.dropEffect = 'move'; stack.classList.add('is-drop-target'); });
  board.addEventListener('dragleave', (event) => event.target.closest('.palette-stack')?.classList.remove('is-drop-target'));
  board.addEventListener('drop', (event) => { const stack = event.target.closest('.palette-stack'); if (!stack || !draggedId) return; event.preventDefault(); stack.classList.remove('is-drop-target'); const groupIndex = Number(stack.dataset.groupIndex); const target = event.target.closest('.palette-shell'); const index = target ? [...stack.querySelectorAll('.palette-shell')].indexOf(target) + (event.clientY > target.getBoundingClientRect().top + target.offsetHeight / 2 ? 1 : 0) : layout.groups[groupIndex].length; moveDrop(draggedId, groupIndex, index); });
  board.addEventListener('pointerdown', (event) => { const column = event.target.closest('[data-palette-column-resize]'); const height = event.target.closest('[data-palette-resize]'); if (!column && !height) return; const handle = column || height; handle.setPointerCapture(event.pointerId); resizeSession = column ? {kind: 'column', index: Number(column.dataset.paletteColumnResize), last: event.clientX, pointerId: event.pointerId} : {kind: 'height', id: height.dataset.paletteResize, last: event.clientY, pointerId: event.pointerId}; });
  board.addEventListener('pointermove', (event) => { if (!resizeSession || resizeSession.pointerId !== event.pointerId) return; const next = resizeSession.kind === 'column' ? event.clientX : event.clientY; const delta = next - resizeSession.last; if (!delta) return; if (resizeSession.kind === 'column') updateColumnWidth(resizeSession.index, delta); else updateHeight(resizeSession.id, delta); resizeSession.last = next; });
  function finishResize(event) { if (!resizeSession || resizeSession.pointerId !== event.pointerId) return; const finished = resizeSession; resizeSession = undefined; save(); say(finished.kind === 'column' ? 'Palette column widths updated.' : `${title(finished.id)} palette height updated.`); }
  board.addEventListener('pointerup', finishResize); board.addEventListener('pointercancel', finishResize);
  board.addEventListener('keydown', (event) => { const column = event.target.closest('[data-palette-column-resize]'); const height = event.target.closest('[data-palette-resize]'); if (column && ['ArrowLeft', 'ArrowRight'].includes(event.key)) { event.preventDefault(); updateColumnWidth(Number(column.dataset.paletteColumnResize), event.key === 'ArrowLeft' ? -20 : 20); save(); column.focus(); } if (height && ['ArrowUp', 'ArrowDown'].includes(event.key)) { event.preventDefault(); updateHeight(height.dataset.paletteResize, event.key === 'ArrowDown' ? 20 : -20); save(); height.focus(); } });
