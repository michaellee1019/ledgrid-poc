...
# Scene v2 presentation resolution lives alongside the established pipeline
# contracts so the selective reconstruction does not erase runtime APIs still
# used by the manager, receiver, and legacy activation boundary.
from typing import Callable

from animation.core.component_catalog import (
    ComponentCatalog as SceneV2ComponentCatalog,
...
PIPELINE_TRACE = (
    ...
    "resolve_look",
    "resolve_plant_effect_intent",
    "select_palette_policy",
    "compute_component_time",
)
...
class ResolvedScene:
    ...
    digest: str
    descriptor: SceneV2ComponentDescriptor
...
def resolve_scene(
    ...
    catalog: SceneV2ComponentCatalog,
    *,
    ...
    palette = (
        _freeze_scene_v2_mapping({"palette_id": look["palette_id"]})
        if descriptor.palette_policy is PalettePolicy.SEMANTIC
        else None
    )
    plant_inputs = _freeze_scene_v2_mapping({
    ...
    return ResolvedScene(
        canonical_scene=_freeze_scene_v2_mapping(canonical.scene),
        canonical_bytes=canonical.canonical_bytes,
        digest=canonical.identity.digest,
        descriptor=descriptor,
        parameters=_freeze_scene_v2_mapping(canonical.scene["animation"]["parameters"]),
        palette=palette,
...
def _freeze_scene_v2_mapping(value: Mapping[str, Any]) -> Mapping[str, Any]:
    return MappingProxyType({
        key: _freeze_scene_v2_json(item) for key, item in value.items()
    })


def _freeze_scene_v2_json(value: Any) -> Any:
    if isinstance(value, Mapping):
        return _freeze_scene_v2_mapping(value)
    if isinstance(value, list):
        return tuple(_freeze_scene_v2_json(item) for item in value)
    return value
