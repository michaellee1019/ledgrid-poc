...
constexpr uint8_t kPaletteCount = 4;
constexpr uint32_t kMaximumCadencePeriodUs = 1000000U;
...
// Canonical Scene roles in ABI order: background_low, background_mid,
// background_high, primary, secondary, accent, hud, warning.
constexpr uint8_t
    kCanonicalPalettes[kPaletteCount][LEDGRID_NATIVE_BACKGROUND_PALETTE_ROLES][3] = {
        {
            {2, 10, 18}, {13, 79, 75}, {24, 148, 132}, {24, 148, 132},
            {87, 202, 175}, {150, 255, 218}, {150, 255, 218}, {255, 202, 92},
        },
        {
            {3, 9, 20}, {22, 56, 81}, {40, 102, 142}, {40, 102, 142},
            {105, 165, 194}, {170, 228, 245}, {170, 228, 245}, {255, 202, 92},
        },
        {
            {15, 3, 34}, {50, 21, 114}, {84, 38, 194}, {84, 38, 194},
            {69, 138, 212}, {54, 238, 230}, {54, 238, 230}, {255, 202, 92},
        },
        {
            {18, 3, 2}, {87, 23, 8}, {156, 42, 14}, {156, 42, 14},
            {206, 122, 53}, {255, 202, 92}, {255, 202, 92}, {255, 72, 64},
        },
    };
...
ledgrid_native_parameter_v2 g_parameters[3];
...
int16_t sin_q15(uint16_t phase) {
  switch (phase) {
    case 0:
    case 32768: return 0;
    case 16384: return 32767;
    case 49152: return -32767;
    default: break;
  }
  constexpr double kTau = 0x1.921fb54442d18p+2;
  return static_cast<int16_t>(round_to_nearest_even(
      std::sin(static_cast<double>(phase) * kTau / 65536.0) * 32767.0));
}
...
void select_palette(uint8_t palette_index) {
  std::memcpy(g_vibe.palette, kCanonicalPalettes[palette_index],
              sizeof(g_vibe.palette));
}
...
void reset_context_values() {
  ...
  g_parameters[0].value.real = 0.72F;
  g_parameters[1].id = 1;
  g_parameters[1].type = LEDGRID_NATIVE_PARAMETER_INT32;
  g_parameters[1].value.integer = 8012;
  g_parameters[2].id = 2;
  g_parameters[2].type = LEDGRID_NATIVE_PARAMETER_FLOAT32;
  g_parameters[2].value.real = 30.0F;
...
LG_BROWSER_EXPORT int lg_browser_init(uint32_t global_strips,
  ...
  for (uint8_t receiver = 0; receiver < kReceiverCount; ++receiver) {
    ...
    auto& profile = g_profiles[receiver];
    profile = {};
...
LG_BROWSER_EXPORT int lg_browser_set_parameters(float gain, int32_t seed,
                                                float source_fps) {
  if (g_initialized_receivers != kReceiverCount || !std::isfinite(gain) ||
      gain < 0.0F || gain > 1.0F || seed < 0 || !std::isfinite(source_fps) ||
      source_fps < 1.0F || source_fps > 60.0F) {
    g_last_error = kBrowserBadParameters;
    return -1;
  }
  g_parameters[0].value.real = gain;
  g_parameters[1].value.integer = seed;
  g_parameters[2].value.real = source_fps;
  return update_all_contexts();
}

LG_BROWSER_EXPORT int lg_browser_set_palette(uint32_t palette_index) {
  if (g_initialized_receivers != kReceiverCount || palette_index >= kPaletteCount) {
    g_last_error = kBrowserBadParameters;
    return -1;
  }
  select_palette(static_cast<uint8_t>(palette_index));
  return update_all_contexts();
}
...
LG_BROWSER_EXPORT int lg_browser_render(uint32_t scene_time_low,
  ...
  for (uint8_t receiver = 0; receiver < kReceiverCount; ++receiver) {
    ...
    // Composer supplies the already-scaled phase clock. Scene presentation is
    // applied after composition, so both clocks are the same in this local preview.
    ...
    if (result.struct_size != sizeof(result) || result.changed > 1U ||
        ...
        result.next_deadline_scene_time_us >
            scene_time_us + kMaximumCadencePeriodUs ||
