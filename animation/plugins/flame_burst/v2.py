...
class FlameBurstAnimation(AnimationBase):
    ...
    def __init__(self, controller: Any, config: Mapping[str, Any] | None = None):
        ...
        self.width, self.height = self.get_strip_info(); self._pixels = np.zeros((self.get_pixel_count(), 4), dtype=np.uint8)
    ...
    def on_presentation_context_changed(self, old: ResolvedScene | None, new: ResolvedScene) -> None:
        ...
        self._presentation_only_refresh = bool(old is not None and old.phase_time == new.phase_time and old.parameters == new.parameters and old.palette != new.palette)
    ...
    def generate_frame(self, time_elapsed: float, frame_count: int) -> OverlayFrame:
        ...
        elif self._presentation_only_refresh: self._presentation_only_refresh=False
        else:
        ...
        if key==self._last_render_key: return OverlayFrame(self._pixels,revision=self._last_tick or 0,changed=False,dirty_ranges=())
    ...
    def _paint(self,palette_id:str)->None:
        low,hot,white=self.SEMANTIC_PALETTES.get(palette_id,self.SEMANTIC_PALETTES["neutral"]); canvas=self._pixels.reshape(self.width,self.height,4); canvas.fill(0); field=np.zeros((self.width,self.height),dtype=np.float32); life=.32+self.params["ember_linger"]*1.5
        ...
        alpha8=np.rint(field*.92*255.).astype(np.uint8)
        for channel in range(3):
            color=np.clip(low[channel]+(hot[channel]-low[channel])*field+(white[channel]-hot[channel])*np.square(field),0.,255.)
            canvas[:,:,channel]=np.floor(color*alpha8/255.).astype(np.uint8)
        canvas[:,:,3]=alpha8
    @classmethod
