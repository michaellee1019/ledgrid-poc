...
class PlantAwareWorldFlagsTests(unittest.TestCase):
    ...
    def test_single_flag_moves_away_from_foliage_and_globe_landmarks(self):
        # The baseline 21-pixel flag is centered at rows 29..49. Cover that entire
        ...
        baseline = WorldFlagsAnimation(_Controller(), common)
        aware = WorldFlagsAnimation(_Controller(), {**common, "plant_aware": True})

        baseline_frame = baseline.generate_frame(0.0, 0).reshape((32, 80, 3))
        aware_frame = aware.generate_frame(0.0, 0).reshape((32, 80, 3))
        mask = np.zeros((32, 80), dtype=bool)
        for index in center:
            mask[index // 80, index % 80] = True

        self.assertGreater(np.count_nonzero(np.any(baseline_frame[mask] != 0, axis=1)), 600)
    ...
    def test_parade_shrinks_and_shifts_a_banner_out_of_a_blocked_slot(self):
        ...
        baseline_overlap = np.count_nonzero(np.any(baseline_frame[blocked_mask] != 0, axis=1))
        aware_overlap = np.count_nonzero(np.any(aware_frame[blocked_mask] != 0, axis=1))

        self.assertGreater(baseline_overlap, 300)
        self.assertLess(aware_overlap, baseline_overlap)
        self.assertLess(aware.get_runtime_stats()["plant_banner_overlap"], baseline_overlap)
