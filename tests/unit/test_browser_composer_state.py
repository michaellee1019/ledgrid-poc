...
""")
    ...
    def test_import_and_keyboard_contracts_are_bounded_and_explicit(self) -> None:
        ...
        self.assertIn("if (state.connectivityPromise) return state.connectivityPromise", source)
        self.assertIn("function adoptMatchingServerCatalog(payload)", source)
        self.assertIn("payload?.catalog_digest !== localDigest", source)
