...
class HeterogeneousTopologyTests(unittest.TestCase):
    def test_fec_allowlist_reaches_only_configured_logical_receiver(self):
        ...
        self.assertEqual(dispatched, [0, 1, 2, 3, 4])
    ...
    def test_fec_full_frame_waits_for_cross_bus_prefixes_before_spi1_suffix(self):
        with (
            mock.patch("drivers.multi_device.LEDController", _Device),
            mock.patch.object(
                MultiDeviceLEDController,
            ...
            ordinary = MultiDeviceLEDController(
                num_devices=5,
                device_map=ROUTES,
                receiver_strip_counts=WIDTHS,
                receiver_global_strip_offsets=OFFSETS,
        ...
        try:
            self.assertEqual(
                controller._full_frame_dispatch_phases(),
                (((0, 1), (2,)), ((3, 4),)),
            )
            self.assertEqual(
                ordinary._full_frame_dispatch_phases(),
                (((0, 1), (2, 3, 4)), ()),
            )
            aggregate = controller.get_stats()["aggregate"]
            self.assertTrue(aggregate["fec_isolated_full_frame_dispatch"])
            self.assertEqual(
                aggregate["full_frame_dispatch_phases"],
                [[[0, 1], [2]], [[3, 4]]],
            )
            controller._display_ownership_known = True
            dispatched = []

            def record(device_ids, *_args):
                dispatched.append(tuple(device_ids))
                return True

            with mock.patch.object(
                controller, "_send_bus_frames", side_effect=record
            ):
                controller.set_all_pixels(
                    np.zeros((33 * 138, 3), dtype=np.uint8)
                )
            self.assertEqual(set(dispatched[:2]), {(0, 1), (2,)})
            self.assertEqual(dispatched[2:], [(3, 4)])

            dispatched.clear()
            with mock.patch.object(
                controller, "_send_bus_partial", side_effect=record
            ):
                controller._send_partial_frame_dispatch([], {}, 7)
            self.assertEqual(set(dispatched[:2]), {(0, 1), (2,)})
            self.assertEqual(dispatched[2:], [(3, 4)])
        finally:
            controller.close()
