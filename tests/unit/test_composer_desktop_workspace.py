...
class ComposerDesktopWorkspaceTests(unittest.TestCase):
    ...
    def test_scene_v2_edits_publish_and_dialogs_have_keyboard_contracts(self) -> None:
        ...
        self.assertIn("${api}/${stop ? 'stop' : 'go-live'}", self.script)
    ...
    def test_global_speed_is_a_single_persistent_scene_pace_control(self) -> None:
        ...
        self.assertIn('Speed <span>Applies to the entire scene', self.html)
        self.assertIn('type="range" min="0" max="2" step=".05" value=".7"', self.html)
        ...
        self.assertIn("controlsWorkspace.querySelectorAll('.inspector')", layout)
        self.assertIn("function setVisibility(id, visible)", layout)
        ...
        self.assertIn("'Hide Global scene'", probe)
        self.assertIn("'Show Global scene'", probe)
        self.assertIn("async function openPaletteChooser(page)", probe)
        self.assertIn("if (!await chooser.evaluate((node) => node.open))", probe)
        self.assertIn("async function closePaletteChooser(page)", probe)
        self.assertIn("if (await chooser.evaluate((node) => node.open))", probe)
        self.assertIn("async function openPaletteArrangeMenu(page, id)", probe)
        self.assertIn('[data-palette-action="collapse"]', probe)
        self.assertIn("locator('xpath=ancestor::*", probe)
        self.assertIn(".closest('.palette-shell')?.querySelector('.palette-arrange-menu')?.open", probe)
