...
class ComposerOfflineShellTests(unittest.TestCase):
    ...
    def test_worker_precaches_only_versioned_shell_assets_and_never_api_state(self) -> None:
        self.assertIn("const CACHE_NAME = 'composer-shell-v3'", self.worker)
        ...
        self.assertIn("caches.match('/static/composer/offline.html?v=composer-shell-v3')", self.worker)
    ...
    def test_offline_shell_disables_mutations_and_requires_fresh_reconnect(self) -> None:
        ...
        self.assertIn("navigator.serviceWorker.register('/composer-sw.js?v=composer-shell-v3', {scope:'/'})", self.shell)
