...
DEFAULT_NATIVE_AURORA_OUTPUT = (
    ROOT / "web/static/generated/composer/native_aurora.wasm"
)
...
NATIVE_AURORA_BRIDGE = (
    ROOT / "animation/browser_preview/native/native_aurora_bridge.cpp"
)
NATIVE_AURORA_SOURCE = ROOT / "animation/plugins/native_aurora/native/background.cpp"
...
NATIVE_AURORA_EXPORTS = EXPORTS + ("_lg_browser_set_palette",)
...
def build_native_aurora(
    output: Path = DEFAULT_NATIVE_AURORA_OUTPUT,
    *,
    compiler: str | None = None,
    repo_root: Path = ROOT,
) -> Path:
    """Build the canonical receiver-native Aurora from its authoritative source."""
    ...
    root = repo_root.resolve()
    sources = (
        root / NATIVE_AURORA_BRIDGE.relative_to(ROOT),
        root / NATIVE_AURORA_SOURCE.relative_to(ROOT),
    )
    include = root / ABI_INCLUDE.relative_to(ROOT)
    for path in sources:
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(
                f"native-Aurora browser source is missing or invalid: {path}"
            )
    ...
    command = (
        ...
        "-ffunction-sections",
        "-fdata-sections",
        "-frandom-seed=ledgrid-browser-native-aurora-v2",
        "-DLG_HOST_PREVIEW=1",
        "-I",
        str(include),
        "-sSTANDALONE_WASM=1",
        ...
        "-sASSERTIONS=0",
        "-sERROR_ON_UNDEFINED_SYMBOLS=1",
        f"-sEXPORTED_FUNCTIONS={','.join(NATIVE_AURORA_EXPORTS)}",
        "-Wl,--no-entry",
        "-Wl,--strip-all",
        "-o",
        str(output),
    ...
    if completed.returncode:
        detail = (completed.stderr or completed.stdout).strip()
        raise RuntimeError(f"native-Aurora browser build failed: {detail}")
    if output.read_bytes()[:4] != b"\0asm":
        raise RuntimeError(
            "native-Aurora browser build did not produce a WebAssembly module"
        )
    output.chmod(0o644)
    return output
...
def main() -> int:
    ...
    parser.add_argument(
        "--native-aurora-output",
        type=Path,
        default=DEFAULT_NATIVE_AURORA_OUTPUT,
    )
    parser.add_argument(
    ...
    native_aurora_output = build_native_aurora(args.native_aurora_output)
    if args.check:
        with tempfile.TemporaryDirectory(
            ...
            native_aurora_second = build_native_aurora(
                Path(name) / native_aurora_output.name
            )
            if native_aurora_output.read_bytes() != native_aurora_second.read_bytes():
                raise RuntimeError(
                    "repeated native-Aurora browser builds are not byte-identical"
                )
    ...
    print(
        f"{native_aurora_output} {native_aurora_output.stat().st_size} bytes "
        f"sha256={sha256(native_aurora_output)}"
    )
    return 0
